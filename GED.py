...
for i in range(0, len(misspell)):
    ...
    for j in range(0, len(dictionary)):
        if abs(len(misspell[i])-len(dictionary[j])) > 5:
            ...
            if weighted_levenshtein.lev(misspell[i], dictionary[j],  delete_costs=delete_costs, insert_costs=insert_costs) < temp_dis:
                temp_dis = weighted_levenshtein.lev(misspell[i], dictionary[j], delete_costs=delete_costs, insert_costs=insert_costs)
                temp_word = str(dictionary[j])
                count = 1
            elif weighted_levenshtein.lev(misspell[i], dictionary[j], delete_costs=delete_costs, insert_costs=insert_costs) == temp_dis:
...
with open('data/0331_output.txt', 'w') as out:
