...
for i in range(0, len(misspell)):
    ...
    for j in range(0, len(dictionary)):
        if abs(len(misspell[i])-len(dictionary[j])) > 5:
            ...
            if Levenshtein.distance(misspell[i], dictionary[j]) < temp_dis:
                temp_dis = Levenshtein.distance(misspell[i], dictionary[j])
                temp_word = str(dictionary[j])
                count = 1
            elif Levenshtein.distance(misspell[i], dictionary[j]) == temp_dis:
...
with open('data/ged_output.txt', 'w') as out:
